            ...
            try
            {
                ...
                else
                {
                    httpConnection = _session.Settings.RenewHttpConnection
                        ...
                        : new HttpConnection(_session.Settings);// _session.GetHttpConnection();

                    //response = await httpConnection.HttpClient.SendAsync(request.RequestMessage);//, cancellationToken);//.ConfigureAwait(false);
                    response = await (new HttpClient()).GetAsync(request.RequestMessage.RequestUri);//, cancellationToken);//.ConfigureAwait(false);
