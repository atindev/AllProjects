    ...
    public class Handler : IRequestHandler<Request, Response>
    {
        ...
        public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var subscription = await $"{_wASApiSettings.ApiBaseUrl}/{EndPointSettings.Unsubscribe}/{request.OfficialEmail}"
                    .WithOAuthBearerToken(token)
                    .DeleteAsync(cancellationToken);

                if (!subscription.IsSuccessStatusCode)
                    throw new NotFoundException($"There is no subscription with the email id {request.OfficialEmail}");
