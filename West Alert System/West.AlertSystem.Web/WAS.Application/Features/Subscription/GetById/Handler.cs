    ...
    public class Handler : IRequestHandler<Request, Response>
    {
        ...
        public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var subscription = await $"{_wASApiSettings.ApiBaseUrl}/{EndPointSettings.GetSubscriberById}/{request.Id}"
                         .WithOAuthBearerToken(token)
                         .GetJsonAsync<Common.Models.Subscription>(cancellationToken);

                if (subscription == null)
                    throw new NotFoundException($"There is no subscription with the id {request.Id}");
