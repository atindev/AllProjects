    ...
    public class Handler : IRequestHandler<Request, Response>
    {
        ...
        public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var attachment = await $"{_wASApiSettings.ApiBaseUrl}/{EndPointSettings.GetAttachment}?FileName={request.FileName}&NotificationEmailId={request.NotificationEmailId}"
                .WithOAuthBearerToken(token)
                .GetJsonAsync<Response>(cancellationToken);

                attachment.ContentDisposition = new System.Net.Mime.ContentDisposition
                {
